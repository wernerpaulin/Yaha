...
class wsManager:
    ...
    def update(self, PDI):
        if (self.oldMinute != datetime.datetime.now().minute):
            ...
            self.cnt15Minutes = self.cnt15Minutes + 1
            if self.cnt15Minutes >= 15:
                self.new15MinuteFlag = True  
                self.cnt15Minutes = 0
    ...
    def requestDataThread(self):
        ...
        try:
            ...
            print("Weather station <{0}> http error: {1}".format(self.wsID, e.code))
            self.httpResponseObj.close()
            self.activeState = "sIDLE"
            return        
        except URLError as e:
            print("Weather station <{0}> could not be reached. Reason: {1}".format(self.wsID, e.reason))
            self.httpResponseObj.close()
            self.activeState = "sIDLE"
            return        
        else:
    ...
    def sIDLE(self):
        if (self.newMinuteRequestData == True):
            self.newMinuteRequestData = False
            self.activeState = "sWS_DATA_REQUEST"

        if (self.new15MinuteFlag == True):
            self.new15MinuteFlag = False
            ...
            elif (self.outputs.actTempOutdoor.value() < self.actTempOutdoorOld):
            ...
            elif (self.outputs.relativePressure.value() < self.relativePressureOld):
            ...
            elif (self.outputs.uvRadiationIndex.value() < self.uvRadiationIndexOld):
            ...
            elif (self.outputs.actHumidityOutdoor.value() < self.uvRadiationIndexOld):

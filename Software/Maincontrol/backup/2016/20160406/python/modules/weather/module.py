    ...
    try:
        ...
        for wsCfg in cfgRoot.findall(WS_CFG_ELEMENT_NAME):
            try:
                ...
                #initialize logging
                cfgItem = wsCfg.find('.//parameters/logging')
                #continue only if configuration exists
                if cfgItem != None:
                    WeatherStations[wsID].param.logging.dir = cfgItem.attrib['dir']
                    WeatherStations[wsID].param.logging.interval = float(cfgItem.attrib['interval'])
    ...
    def sWS_DATA_RESPONSE_EXTRACT_DATA(self):
        ...
        if (self.outputs.uvRadiation.value() >= 1000):
            ...
            if (self.outputs.rainHourly.value() > 0):
                if (self.actTempOutdoor.value() > -2.0):

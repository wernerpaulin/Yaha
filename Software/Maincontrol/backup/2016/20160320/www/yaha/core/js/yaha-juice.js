...
- anim_svg_item_ids: ids of item in svg graphics whose property should be manipulated
- anim_value_ids: ids of tags with the value which controls the animation
		...
		yahaPropertyList = attrString.split(';')
	 	...
	 	for (p in yahaPropertyList) 	//go through each definition: property:value1,value2
	 	{
			...
			switch(propertyName)
			{
				...
				break;
				case "anim_svg_item_ids":
					AnimationSvgItemIDs[$(this).prop('id')] = propertyValueList			//add ids of item in svg graphics whose property should be manipulated
				break;
				case "anim_value_ids":
					AnimationValueIDs[$(this).prop('id')] = propertyValueList			//add ids of tags with the value which controls the animation
				break;
				case "anim_attributes":
...
function postUpdateUi(jsonData)
{
	...
	for (tag in dataFromServer)
	{
		...
		if (tag in AnimationSvgItemIDs && tag in AnimationValueIDs && tag in AnimationAttributes)		//minimum requirements for an animation
		{
...
/*
		...
		//access SVG item and change attribute;
		svgDoc.getElementById("actBlindPos").setAttribute("height", "500");
		
		AnimationValueIDs[$(this).prop('id')] = propertyValueList			//add symbol mapping of tag which should be applied when received from server
		AnimationAttributes[$(this).prop('id')] = propertyValueList			//add symbol mapping of tag which should be applied when received from server
		AnimationAttributeValueMin[$(this).prop('id')] = propertyValueList			//add symbol mapping of tag which should be applied when received from server
		AnimationAttributeValueMax[$(this).prop('id')] = propertyValueList			//add symbol mapping of tag which should be applied when received from server
		AnimationValueMin[$(this).prop('id')] = propertyValueList			//add symbol mapping of tag which should be applied when received from server
		AnimationValueMax[$(this).prop('id')] = propertyValueList			//add symbol mapping of tag which should be applied when received from server
*/
	} //for (tag in dataFromServer)

...
class yahaEnoceanIOwrapper():
        ...
        for tag in self.ioTags:
            ...
            try:
                ...
                if idStringElements[self.DIRECTION_INDEX].lower() == "in":
                    ...
                    self.readTags[tag].functionVariable = idStringElements[self.FUNCTION_VARIABLE]

                    self.readTags[tag].enoceanDevice = self.enoceanManager.addDevice(self.readTags[tag].moduleID)
                    ...
                    self.writeTags[tag].functionVariable = idStringElements[self.FUNCTION_VARIABLE]

                    self.writeTags[tag].enoceanDevice = self.enoceanManager.addDevice(self.writeTags[tag].moduleID)
    ...
    def readInputs(self):
        ...
        #go through all tags which should be read and write current value directly into PDI

...
#get all different owners of PDI tags (e.g. used for IO-scheduler)
def YahaPDIgetHostIDs(owner):
    hostIDlist = {'empty': 0}    #force creating a dictionary
    hostIDlist.clear()           #clear dictionary immediately
    ...
    if records is None:
        ...
        for record in records:
            try:
                hostIDlist[record[processTag]] = record[owner].split('.')[1] #"tag" : owner.aabbccdd => tag : aabbccdd 
            except:
                hostIDlist[record[processTag]] = None   #no specific host ID defined => use standard ID of driver later on
...
class ProcessDataImage:
    ...
    def getTagHostIDs(self, owner):
        return(YahaPDIgetHostIDs(owner))

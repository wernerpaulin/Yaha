...
var CyclicReadTimer
var SuspendCyclicReadCnt = 0
var InhibitReadWriteCnt = 0
...
function yjAcquireReadWriteInhibit()
{
	InhibitReadWriteCnt += 1
}

function yjReleaseReadWriteInhibit()
{
	if (InhibitReadWriteCnt > 0)
	{
		InhibitReadWriteCnt = InhibitReadWriteCnt - 1
	}
}

function yjReadWriteInhibit()
{
	return ((InhibitReadWriteCnt > 0) ? true : false)
}
...
function yjCylicRefresh()
{
	...
	//no CGI calls in case inhibit has been requested (e.g. other CGI calls are active)
	if (yjReadWriteInhibit() == true) 
	{
		console.log("Yaha juice: read / write inhibited due to other CGI calls")
		return;		
	}

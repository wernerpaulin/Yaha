...
def calcCRC8sum(byteStream):
    CRCsum = 0
    for c in byteStream:
        CRCsum = crc8Table[CRCsum & 0xFF ^ c & 0xFF]
    return CRCsum  

#check given checksum with calculated one            
def checkCRC8correct(byteStream, crc8SumGiven):
    if crc8SumGiven == calcCRC8sum(byteStream):
        return True
    else:
        return False

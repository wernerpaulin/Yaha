...
class main():
    ...
    def addDevice(self, deviceID='ffffffff', hostIDoverride=None):
        #create device
        ...
        if not deviceID in self.deviceList:
            ...
            if hostIDoverride <> None:
                self.deviceList[deviceID].packetTypeRadio.hostID = hostIDoverride  #Use special host ID which is individually defined for certain data points in case the destination can not handle one common host for several actors
            else:

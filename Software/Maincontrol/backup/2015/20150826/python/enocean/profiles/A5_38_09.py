...
class A5_38_09():
    ...
    def pack(self, senderID = 'ffffff', destinationID = 'ffffff'):
        ...
        '''
        TODO: A5_38_09 - je nachdem welche Parameter ge�ndert wurden (old) anders packet bauen: gilt auch f�r Teach: nur Learn-bit setzen
        TODO: A5_38_09 - einige Kommandos haben gleiche Werte!
        TODO: A5_38_09 - Standard device light anpassen: Dr�ckdauer wird neu vom EEP profil �bernommen nicht mehr in Statemachine von light
        '''
        #build data bytes (special tgm data in case of teach-in)
        if (self.learning == 1):
            self.learning = 0       #avoid multiple learning telegrams
            data.extend(self.buildTeachInData(senderID, destinationID))
        else:
            data.extend(self.buildOperationalData(senderID, destinationID))
    ...
    def buildTeachInData(self, senderID, destinationID):
        byte0 = 0
        byte1 = 0
        byte2 = 0
        byte3 = 0
    ...
    def buildOperationalData(self, senderID, destinationID):
        byte0 = 0
        byte1 = 0
        byte2 = 0
        byte3 = 0

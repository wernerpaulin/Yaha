...
class yahaEnoceanIOwrapper():
    ...
    def __init__(self, PDI):
        ...
        self.FUNCTION_VARIABLE_INDEX = 3
        ...
        #get all PDI tags of a owner 'enocean' which have a special host ID configured. This is used to teach different actuators with different sender IDs as the often do not allow selective destinationIDs
        self.hostIDs = PDI.getTagHostIDs('enocean')  #"tag" : owner
        ...
        for tag in self.ioTags:
            try:
                ...
                if idStringElements[self.DIRECTION_INDEX].lower() == "in":
                    ...
                    self.readTags[tag].enoceanDevice = self.enoceanManager.addDevice(self.readTags[tag].moduleID, self.readTags[tag].hostIDoverride)
                    self.enoceanManager.addDeviceFunctionType(self.readTags[tag].moduleID, self.readTags[tag].functionType)
                else:
                    self.writeTags[tag] = yahaEnceanIOpoint()
                    self.writeTags[tag].processTag = tag
                    self.writeTags[tag].moduleID = idStringElements[self.MODULE_ID_INDEX]
                    self.writeTags[tag].hostIDoverride = self.hostIDs[tag]
                    ...
                    self.writeTags[tag].enoceanDevice = self.enoceanManager.addDevice(self.writeTags[tag].moduleID, self.writeTags[tag].hostIDoverride)
    ...
    def writeOutputs(self):
        for tag in self.writeTags:
            ...
            if currentValue <> self.writeTags[tag].processTagOldValue:
                self.writeTags[tag].enoceanDevice.packetTypeRadio.setTagValue(self.writeTags[tag].functionType, self.writeTags[tag].functionVariable, currentValue)

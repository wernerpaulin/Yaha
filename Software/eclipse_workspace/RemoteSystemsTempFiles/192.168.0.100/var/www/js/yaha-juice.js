	...
	for (tag in dataFromServer)
	{
		...
		if (tag in SelectDataIDs)		
		{
			try	//fillSelect would fail if tag does not exist anymore due to page change
			{
				...
				{
					fillSelect(tag, SelectDataIDs[tag], SelectValueIDs[SelectDataIDs[tag]])	//tag contains select data; SelectDataIDs[tag] contains the id of the select box, SelectValueIDs[tag] contains the item which should be displayed
				}
				else
				{
					fillSelect(tag, SelectDataIDs[tag], null)	//tag contains select data; SelectDataIDs[tag] contains the id of the select box

...
def init(PDI):
	...
	#read module configuration and initialize each subscriber
	try:
...
class subscriberManager:
	...
	def update(self, PDI):
		...
		if (len(replyText) > 0):
			#add time stamp
			replyText = datetime.datetime.now().strftime("%Y-%m-%d %H:%M") + "\n" + replyText
			#split in 160 character blocks for SMS transfer
			smsChunks = split_len(replyText, MAX_SMS_CHAR)
			#send all chunks
			for smsChunk in smsChunks:
				#print(smsChunk)
				self.smsSendQueue.append(smsChunk)

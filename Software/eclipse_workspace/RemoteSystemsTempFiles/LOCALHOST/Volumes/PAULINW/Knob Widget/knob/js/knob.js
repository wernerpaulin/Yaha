...
function knobWidgetInit(idKnob, idBars, snapToZeroValue, startValue, minValue, maxValue, stepWidthValue, minDegree, maxDegree, barSpacingDegrees, valueTagId)
{
	...
	$('#'+idKnob).knobKnob({
		...
		turn : function(ratio, turnDueToInputFieldChange){
			//update connected tag variable with new value
			writeTagValue(valueTagId, valueRound(calcX(k, d, ratio * 360, minValue, maxValue), stepWidthValue), turnDueToInputFieldChange)
	...
	$.fn.knobKnob = function(props){
		...
		return this.each(function(){
			...
			if(options.degree > 0 && options.degree <= 359){
				...
				turnKnob(knob, currentDeg, options.minDegree, options.maxDegree, options.turn)
			}
			...
			knob.on('mousedown touchstart', function(e){
				...
				knob.on('mousemove.rem touchmove.rem',function(e){
					...
					//turn knob
					lastDeg = turnKnob(knob, currentDeg, options.minDegree, options.maxDegree, options.turn)
				});
			...
			$('#'+options.valueTagId).on('change', function(){
				...
				rotation = lastDeg = turnKnob(knob, newDeg, options.minDegree, options.maxDegree, options.turn)
			});
...
function turnKnob(knobObj, degree, minDegree, maxDegree, callBack)
{
	try
	{

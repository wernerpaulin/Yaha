...
class main:
    ...
    def __init__(self, PDI):
        ...
        try:
            ...
            self.RXsocket.bind((self.myServerAdr, PORT_Client_to_Server))
    ...
    def update(self):
        ...
        if (len(self.txData) == 0):
            try:    #see whether there is data (assume entire frame is there)
                self.jsonDataStream, self.clientIP = self.RXsocket.recvfrom(RX_TX_BUFFER_SIZE)
            ...
            #except Exception as e: #try again next cycle in case no date was there. #Python3
                ...
                #print("Y2Y Server update exception: {0}".format(e))
                self.txData.clear()

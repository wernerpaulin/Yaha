...
EVENT_ACTION_OFF_AT_SUNRISE = "offAtSunrise"
EVENT_ACTION_ON_AT_SUNSET = "onAtSunset"
...
class switchManager:
    ...
    def __init__(self, PDI):
        ...
        self.offAtSunrise = False
        self.onAtSunset = False
        self.sunriseTime = 0
        self.sunsetTime = 0
    ...
    def sSWITCH_OFF_DELAY(self):
        #stop if enable has been removed
        if (self.param.outputmode[OUTPUTMODE_OFFDELAY_NAME].enable.value() == 0):
            self.outputs.off.value(1)
            self.activeState = "sIDLE"
            return
    ...
    def sTIME(self):
        ...
        for event in self.param.timecontrol.eventList:
            ...
            if (calAction == EVENT_ACTION_ON):
            ...
            elif (calAction == EVENT_ACTION_OFF_AT_SUNRISE):
                ...
                self.offAtSunrise = True
        
            elif (calAction == EVENT_ACTION_ON_AT_SUNSET):
                ...
                self.onAtSunset = True
        
        #processing possible set sunrise/sunset flags
        if (self.offAtSunrise == True) and (datetime.datetime.now() >= self.sunriseTime):
            self.offAtSunrise = False
            self.activeState = "sSWITCH_OFF"
            return
        
        elif (self.onAtSunset == True) and (datetime.datetime.now() >= self.sunsetTime):
            self.onAtSunset = False
            self.activeState = "sSWITCH_ON"
            return

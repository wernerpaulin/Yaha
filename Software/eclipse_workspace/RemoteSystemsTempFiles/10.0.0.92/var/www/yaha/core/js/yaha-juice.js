...
var MonitorCyclicReadTagList = new Array()
var MonitorLiveData = {}
var MonitorCgiDataWriteTagList = new Array()
...
var PostUpdateUiFunctionMonitorLiveDataObj
...
var AnimationAttributeValueMap = {}
var AnimationUnitConversionIDs = {}
var AnimationFormatIDs = {}
...
//read list for tags to monitor
function yjDefineMonitorCyclicReadTagList(tagList)
{
	MonitorCyclicReadTagList = tagList;
}

//add a tag + value to be written to server: list has already cgi data format tag:value
function yjAddMonitorTagToWriteCgiData(tagName, tagValue)
{
	MonitorCgiDataWriteTagList[tagName] = tagValue
}
...
function yjAddPostUpdateUiFunctionMonitorLiveData(f)
{
	PostUpdateUiFunctionMonitorLiveDataObj = f
}

function getNbKeysInObj(obj)
{
	nbKeys = 0
	for (key in obj)
	{
...
function yjCylicRefresh()
{
	...
	//give write priority if there are tags from HTML pages in the queure or the monitor requests some tag values to be written
	if ( (WriteTagListQueue.length > 0) || (getNbKeysInObj(MonitorCgiDataWriteTagList) > 0) )
	{
		yjSuspendCyclicRead()	//suspend cyclic read while writing to give writing priority

		//add standard ui tags to be written to server
		cgiData = populateCGIdataToWrite(WriteTagListQueue)
		
		//add monitor values to cgiData
		if (getNbKeysInObj(MonitorCgiDataWriteTagList) > 0)
		{
			for (tag in MonitorCgiDataWriteTagList)
			{
				cgiData[tag] = MonitorCgiDataWriteTagList[tag]
			}

			MonitorCgiDataWriteTagList = {}		//delete all elements of monitor list	
		}	
	...
	else
	{
		...
		//add monitoring data to queue
		ReadTagListQueue = ReadTagListQueue.concat(MonitorCyclicReadTagList);
	...
	function(jsonDataFromServer,status)
    {
		...
		//call back with update monitor data
		if (PostUpdateUiFunctionMonitorLiveDataObj) PostUpdateUiFunctionMonitorLiveDataObj(MonitorLiveData)
    });
}
...
function updateUI(jsonData)
{
	...
	{
		//check if tag received was requested for monitoring
		if (MonitorCyclicReadTagList.indexOf(tag) > -1)
		{
			MonitorLiveData[tag] = dataFromServer[tag]
		}
		...
		if (typeof(element) != 'undefined' && element != null) 
		{
			...
			//skip refresh of user interface if element has focus (user is editing) - not for radio and checkboxes otherwise there is no update
			...
			if (element.nodeName.toLowerCase() == "input")
			{
				...
				if ((element.type == "radio") || (element.type == "checkbox"))
				{
					...
					//fire change event if value has been changed
					if (oldValue != elementValue) jQuery("#"+tag).trigger("change");						
				}
				else
				{
					oldValue = jQuery("#"+tag).val()
					jQuery("#"+tag).val(dataFromServer[tag]);

					//fire change event if value has been changed
					if (oldValue != dataFromServer[tag]) jQuery("#"+tag).trigger("change");
				}
...
- anim_attr_val_map: mapping a value to animation value
- anim_format: leading zeros, decimal placed for numbers in SVG
- anim_unit: conversion and display of units in SVG
...
function yjAttributeProcessor()
{
	...
	AnimationValueMin = {}
	AnimationValueMax = {}
	AnimationAttributeValueMap = {}
	AnimationUnitConversionIDs = {}
	AnimationFormatIDs = {}
		...
		yahaPropertyList = attrString.split(';')
	 	...
	 	for (p in yahaPropertyList) 	//go through each definition: property:value1,value2
	 	{
			...
			switch(propertyName)
			{
				...
				break;
				case "anim_attr_val_map":
					AnimationAttributeValueMap[$(this).prop('id')] = propertyValueList			//add value mapping of tag which should be applied when received from server
				break;
				case "anim_format":
					AnimationFormatIDs[$(this).prop('id')] = propertyValueList[0]			//add format string of tag which should be applied when received from server
				break;
				case "anim_unit":
					AnimationUnitConversionIDs[$(this).prop('id')] = propertyValueList[0]	//add unit conversion string of tag which will be interpreted on update
				break;
...
function postUpdateUi(jsonData)
{
	...
	for (tag in dataFromServer)
	{
		...
		{
			var unitConversionResult = valueUnitConversion(UnitConversionIDs[tag], parseFloat(jQuery("#"+ tag).text()))
			
			jQuery("#"+ tag).text( parseFloat(unitConversionResult.numFloatReturn) )
			showUnit = unitConversionResult.showUnitReturn
			dstUnit = unitConversionResult.dstUnitReturn
		}
		
		//FORMATING: check if for this tag a special formating rule has been defined
		if (tag in FormatIDs)
		{
			jQuery("#"+ tag).text( valueFormating(FormatIDs[tag], parseFloat(jQuery("#"+ tag).text())) )
		}
		...
		if (tag in AnimationTargetIDs)		//check whether this tag should perform an animation on a svg object
		{
			try	//abort animation if parameters are wrong
			{
				...
				else
				{
					...
					catch(err)		//assume it is a text (e.g for color animation, when PDI defines the color)
					{
				...
				{
					if (tag in AnimationAttributeValueMap)
					{
						//loop through all value maps and see which one fits. Stop loop after mapping has been applied
						for (map in AnimationAttributeValueMap[tag])					//1=red, 0=grey
						{
							mapping = AnimationAttributeValueMap[tag][map].split("=")	//1=on
							if (mapping[0] == animationValue)					//index 0 contains value
							{
								attributeValue = mapping[1]		//index 1 contains replacement value or text
								break;
							}
						}
					}
					//scaling only if parameters are given
					else if ( (tag in AnimationValueMin) && (tag in AnimationAttributeValueMin) && (tag in AnimationValueMax) && (tag in AnimationAttributeValueMax) )
					{
						//x = animationValue
				...
				try
				{
					...
					else if (AnimationAttributes[tag][0] == "style")
					{
						if (AnimationAttributes[tag][1] == "fill")
						{
							//access specific item within in SVG and modify a certain attribute: change fill color of element		
							svgDoc.getElementById(AnimationSvgItemIDs[tag]).setAttribute("style", "fill:" + attributeValue);
						}
					}
					else if (AnimationAttributes[tag][0] == "textContent")
					{
						//UNITS: check if for this tag a unit conversion has been defined
						showUnit = false
						dstUnit = ""
						if (tag in AnimationUnitConversionIDs)
						{
							var unitConversionResult = valueUnitConversion(AnimationUnitConversionIDs[tag], attributeValue)
							
							attributeValue = unitConversionResult.numFloatReturn
							showUnit = unitConversionResult.showUnitReturn
							dstUnit = unitConversionResult.dstUnitReturn							
						}
						
						//FORMATING: check if for this tag a special formating rule has been defined
						if (tag in AnimationFormatIDs)
						{
							attributeValue = valueFormating(AnimationFormatIDs[tag], attributeValue)
						}
						...
						if (showUnit == true)
						{
							...
							{
								attributeValue = attributeValue + " " + dstUnit
							}
						}
						
						svgDoc.getElementById(AnimationSvgItemIDs[tag][0]).textContent = attributeValue
					}
					else
					{
				...
				{
					//console.log("SVG animation attribute for " + "<" + tag + ">" + " invalid or missing:" + err.message)
				}
			}
			catch(err) 
			{
...
function valueUnitConversion(unitString, numFloat)		//UnitConversionIDs[tag], numFloat, returns a string
{
	var unitDefinitionTable =	
	   {	"ms" : 0.001 , 								//Table: Unit : relation to base unit (e.g. s)
			"s": 1,
			"min": 60,
			"h" : 3600,
			"day" : 86400,
	...
	if (srcUnit != "" && dstUnit != "")
	{
	    ...
	    return	{
	        		numFloatReturn: numFloat, 
	        		showUnitReturn: showUnit,
	        		dstUnitReturn: dstUnit
	    		}; 		
	}
	else
	{
...
function valueFormating(formatString, numFloat)		//FormatIDs[tag], numFloat, returns a string
{
	//format like: 00.##, .##
	if (formatString.split(".").length > 1)
	{
		//get of leading zeros
		nbLeadingZeros = formatString.split(".")[0].replace(/[^0]/g, "").length  //trick: use reg-exp to remove all characters except "0" and then count the length of the remaining string

		//get number of decimal places
		nbDecimalPlaces = formatString.split(".")[1].length
	}
	//format like 00
	else
	{
		//get of leading zeros
		nbLeadingZeros = formatString.replace(/[^0]/g, "").length  //trick: use reg-exp to remove all characters except "0" and then count the length of the remaining string
	...
	//apply format: decimal places: numFloat = text as float number
	numText = numFloat.toFixed(nbDecimalPlaces)			//trim decimal places and write number as text back
	
	//apply format: leading zeros
	floatString = numText										//keep original float string
	...
	return(floatStringWithLeadingZeros)
}

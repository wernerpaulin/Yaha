...
class circuitManager:
    ...
    def __init__(self, PDI):
        ...
        self.energyRecycleActive = False
    ...
    def sIDLE(self):
        if (self.param.operatingModes.switch.value() == AUTOMATIC_MODE):
            ...
            self.PID.enable = False         #disable PDI to avoid that i-part continues increasing
    ...
    def sAUTOMATIC(self):
        ...
        if (self.inputs.mixerState.value() == MIXER_STATE_HOMING_INVALID):
            self.PID.enable = False         #disable PDI to avoid that i-part continues increasing
            self.outputs.pumpRelease.value(0) #switch off pump until state of mixer is clear            
        ...
        #in case energy recycle mode override pump
        if (self.energyRecycleActive == True):
            self.outputs.pumpRelease.value(1)     
    ...
    def roomController(self):
        #a room controller is a 2-point controller with hysteresis and switches of if a desired room temperature has been reached.
        #In this case it will give 0°C set point to the flow controller which force the PID to close the mixer
    ...
    def flowController(self):
        #However in case the burner has still some heat energy and no one else needs it (setTemperatureBurner == 0) pump this energy into the circuit so it is not wasted 
        #resultingSetTempBurner: burners real set temperature after evaluating all connected devices which needs energy. If 0: no one needs energy
        #as long as the burner is warmer then a certain flow temperature override standard set point (setFlowTemperature) and drain energy
        if (self.status.heatingState.value() != HEATING_STATE_OFF) and (self.inputs.resultingSetTempBurner.value() == 0) and (self.inputs.actTempBurner.value() >= self.param.settings.energyRecycleFlowTemp):
            self.energyRecycleActive = True
            setFlowTemperature = self.param.settings.energyRecycleFlowTemp
        else:
            #the set point of the flow controller is calculated by the room controller
            setFlowTemperature = self.status.setFlowTemperature.value()  
            self.energyRecycleActive = False

        
        #PID controller for flow temperature
        if (setFlowTemperature != 0):
...
class circuitInputs():
    ...
    def __init__(self, PDI):
        ...
        self.resultingSetTempBurner = core.modules.iosignal.IOsignal(PDI)
...
class paramSettings:
    ...
    def __init__(self):
        ...
        self.energyRecycleFlowTemp = 100.0          #higher as burner can be: this deaktivates this function by default

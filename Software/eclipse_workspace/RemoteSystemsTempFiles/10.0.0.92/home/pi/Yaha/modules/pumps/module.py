...
class pumpManager:
    ...
    def __init__(self, PDI):
        ...
        self.statisticsSwitchOnDuration = 0
        
        self.statemachine = {
                             "sIDLE": self.sIDLE,
                             "sAUTOMATIC": self.sAUTOMATIC,
                             "sAUTOMATICmaintenanceSwitchOn": self.sAUTOMATICmaintenanceSwitchOn,
        ...
        self.stopTimer = 0
        self.maintenanceSwitchOnTimer = 0
    ...
    def update(self, PDI):
        ...
        if (self.outputs.release.value() == 1):
            self.status.state.value(PUMP_STATE_ON)

            #log on duration with a 1-second resolution
            self.statisticsSwitchOnDuration = self.statisticsSwitchOnDuration + self.samplingTime

            #reset stop timer            
            self.stopTimer = 0
        else:
            self.status.state.value(PUMP_STATE_OFF)
            self.startTimeRelease = time.time()    #updating will be stopped when release
            self.stopTimer = self.stopTimer + self.samplingTime     #count time when pump is stopped
        ...
        if (self.oldHour != datetime.datetime.now().hour):
            self.oldHour = datetime.datetime.now().hour
            self.status.annualOperatingHours.value(self.status.annualOperatingHours.value() + float(self.statisticsSwitchOnDuration) / 3600)
            self.status.totalOperatingHours.value(self.status.totalOperatingHours.value() + float(self.statisticsSwitchOnDuration) / 3600)
            self.statisticsSwitchOnDuration = 0
  
        #reset annual operating hour counter in case of a new year
        if (self.currentYear != datetime.datetime.now().year):
            self.status.annualOperatingHours.value(0)
            self.currentYear = datetime.datetime.now().year
    ...
    def sAUTOMATIC(self):
        ...
        #optional: start pump for maintenance if it is stopped for a certain time
        try:
            if (self.param.settings.maxStopTime != None):
                #initiate maintenance if stopped for a long time but wait until midnight (burner is usually cool) 
                if (self.stopTimer >= self.param.settings.maxStopTime) and (datetime.datetime.now().hour == 0) and (datetime.datetime.now().minute == 0):
                    self.maintenanceSwitchOnTimer = 0
                    self.activeState = "sAUTOMATICmaintenanceSwitchOn"
                    return
        except:
            pass
           
    def sAUTOMATICmaintenanceSwitchOn(self):
        #leave automatic mode in case of new mode
        if (self.param.operatingModes.switch.value() != AUTOMATIC_MODE):
            self.activeState = "sIDLE"
            return

        #switch on pump for a certain time
        self.maintenanceSwitchOnTimer = self.maintenanceSwitchOnTimer + self.samplingTime
        if (self.maintenanceSwitchOnTimer < self.param.settings.maintenanceSwitchOnDuration):
...
class paramSettings:
    ...
    def __init__(self):
        ...
        self.maxStopTime = None                 #optional: if not configured it remains none
        self.maintenanceSwitchOnDuration = 0    #optional: if not configured it remains 0

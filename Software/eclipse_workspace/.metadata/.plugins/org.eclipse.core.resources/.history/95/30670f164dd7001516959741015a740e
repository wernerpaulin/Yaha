...
#python /home/pi/Yaha/modules/blinds_shading/module.py
...
class blindManager:
    ...
    Supports anti-glare mode: 
        ...
        Loosing the position or button operation will disable anti-glare mode
    ...
    def sIDLE(self):
        ...
        elif (self.param.antiGlare.enable.value() == 1) :
            #activate anti glare if enabled and homing is valid and not fully closed (opening needs to be done by event or manually)
            if (self.homingValid == True) and (self.status.fullyclosed.value() == 0):
                #evaluate every minute to recalculate position
                if (self.newMinuteAntiGlare == True):
                    self.activeState = "sANTI_GLARE_INIT"
                    self.newMinuteAntiGlare = False

            #start homing by opening if anti glare gets enabled (again) which shutter is open
            elif (self.homingValid == False) and (self.status.fullyclosed.value() == 0):
                self.activeState = "sMOVE_OPEN"
                
        elif (self.param.timecontrol.enable.value() == 1):
            #evaluate every minute whether an event is active
            if (self.newMinuteTimeControl == True):
    ...
    def sANTI_GLARE_INIT(self):
        print("ANTI GLARE NOT IMPLEMENTED")
        self.activeState = "sIDLE"

...
class PacketTypeRadio():
    ...
    def unpack(self, data, optData):
        try:
            ...
            print("EEP type <{0}> or sub type <{1}> is not supported".format(self.type, self.subType))
    ...
    def pack(self, subType, subTypeOptionString):
        ...
        try:
            ...
            print("EEP pack function returned no or invalid data: tag <{0}>: {1}".format(self.subTypePack, err))
    ...
    def getTagValue(self, subType, tag):
        try:
            ...
            print("EEP get: tag <{0}> does not exist in sub type <{1}>: {2}".format(tag, subType, err))
            return

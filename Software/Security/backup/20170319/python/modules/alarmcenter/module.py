...
ALARM_CENTER_STATE_ARMED = 1

ZONE_STATE_DISABLED = 0
ZONE_SToffDelayLARM = 1
ZONE_STATE_ALARM = 2
...
class alarmCenterManager:
	...
	def update(self, PDI):
		...
		if (self.oldAlarmCenterState != self.status.state.value()):
			if (self.status.state.value() == ALARM_CENTER_STATE_ARMED):
		...
		for zoneID in self.zones:
			self.zones[zoneID].alarmCenterArmed = self.status.state.value()
			...
			if (self.zones[zoneID].status.state.value() == ZONE_STATE_ALARM) and (self.zones[zoneID].oldState != ZONE_STATE_ALARM):
				self.alarmLight.activate(offDelay=None, onDelay=self.settings.alarmDelayLight.value())
				self.alarmSound.activate(offDelay=None, onDelay=self.settings.alarmDelaySound.value())
			elif (self.zones[zoneID].status.state.value() == ZONE_STATE_NO_ALARM) and (self.zones[zoneID].oldState != ZONE_STATE_ALARM):
				self.alarmLight.deactivate()
				self.alarmSound.deactivate()
			
			"""
			WEITER MACHEN: 
			self.settings.delayToBeArmed
			Testen der Light und Soundverzögerung
			"ICONS für Button definieren"			
			"""
...
class alarmCenterZone:
	...
	def update(self):
		...
		print("oldState: intern: status: stateChange = true und den dann ausserhalb verwenden")
		
		#skip zone if deactivated
		if (self.inputs.enable.value() == 0) or (self.alarmCenterArmed == 0):
...
class alarmSignalManager:
	...
	def update(self):
		...
		#handle onDelay of alarm signal if con
		if (self.onDelay != None):
		...
		#reset onDelayTimer to be prepared for next switchh on
		if (self.signalState == 0):
			self.onDelayTimer = 0

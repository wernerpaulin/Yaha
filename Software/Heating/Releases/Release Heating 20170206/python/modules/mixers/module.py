...
class mixerManager:
    ...
    def update(self, PDI):
        ...
        #cyclic set of set position to allow mixer to abort current movement in case PID changes direction
        if (self.param.operatingModes.switch.value() == AUTOMATIC_MODE):
            #limit movement range of mixer because when motor reaches the mechanical limit it keeps on making noises
            self.status.setPosition.value( min(max(self.param.operatingModes.modes.automatic.setPosition.value(), self.param.settings.minPosition), self.param.settings.maxPosition)  )
        else:
            #in manual mode the entire movement range can be used
        ...
        #mixer actual position generation
        deltaPosPerSample = (self.param.settings.fullyOpenPosition - self.param.settings.fullyClosedPosition) / self.param.settings.fullyOpenTime * self.samplingTime
        
        #mixer state generation
        #not yet homed
        if (self.homingValid != True):
            self.status.state.value(MIXER_STATE_HOMING_INVALID)
            self.newActPosition = self.param.settings.fullyClosedPosition
        #opening
        elif (self.outputs.open.value() == 1):
            self.status.state.value(MIXER_STATE_OPENING)
            self.newActPosition = self.newActPosition + deltaPosPerSample
        #closing    
    ...
    def sAUTOMATIC(self):
        ...
        if (abs(self.status.setPosition.value() - self.status.actPosition.value()) >= self.param.settings.zeroWindow):
    ...
    def sMANUAL(self):
        ...
        if (self.param.operatingModes.modes.manual.cmdHoming.value() == 1):
            self.homingValid = False
            self.param.operatingModes.modes.manual.cmdHoming.value(0)
    ...
    def sHOMING(self):
        self.homingMovementDuration = self.homingMovementDuration + self.samplingTime
        
        #close mixer 10% longer than time to fully open mixer
        if (self.homingMovementDuration > (self.param.settings.fullyOpenTime * 1.1)):
            self.homingValid = True                                 #set homing flag
            self.newActPosition = self.param.settings.fullyClosedPosition                     
            self.status.actPosition.value(self.newActPosition)

            #open mixer slightly again to avoid resting at mechanical limit of mixer motor (noise)            
            self.status.setPosition.value(self.param.settings.minPosition)
            self.activeState = "sMOVE_OPEN"             #try to open mixer - at least approach minPosition in automatic mode to avoid staying at mechanical limit of mixer motor 
            return
        else:
            self.outputs.close.value(1)       #close mixer for homing: safer then opening the mixer
            self.outputs.open.value(0)
    ...
    def sMOVE_OPEN(self):
        ...
        if (self.homingValid != True) or (self.status.actPosition.value() >= self.status.setPosition.value()) or (self.operatingModeBeforeMovement != self.param.operatingModes.switch.value()):
            self.activeState = "sMOVE_STOP"
            return

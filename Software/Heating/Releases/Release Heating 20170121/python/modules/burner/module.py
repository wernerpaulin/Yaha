...
class burnerManager:
    ...
    def update(self, PDI):
        ...
        if (self.inputs.overTemperature.value() == 1) or (self.inputs.actTemp.value() > self.param.settings.safetySwitchOffTemperature):
        ...
        if (self.inputs.switchedOn.value() == 1):
            ...
            #log firing duration with a 1-second resolution
        ...
        if (self.oldHour != datetime.datetime.now().hour):
            self.oldHour = datetime.datetime.now().hour
            self.status.dailyOperatingHours.value(self.status.dailyOperatingHours.value() + float(self.statisticsSwitchOnDuration) / 3600)
        ...
        if (self.currentDay != datetime.datetime.now().day):
            self.status.dailyOperatingHours.value(0)
            self.currentDay = datetime.datetime.now().day
    ...
    def sSTOP(self):
        #shut down burner
        self.outputs.release.value(0)
        #stay in stop state until all alarms are gone and temperature is at least 10% lower than switch off temperature
        if (self.status.alarmOvertemperature.value() == 0) and (self.status.alarmGeneralFailure.value() == 0) and (self.inputs.actTemp.value() < self.param.settings.safetySwitchOffTemperature * 0.9):
            self.activeState = "sIDLE"
            return
    ...
    def simulationLogic(self):
        ...
        self.inputs.actTemp.value(self.simTemperaturePT2.update(self.simTargetTemp, self.param.simulation.settings.burnerTemperaturePtTau, 1))

    ...
    try:
        ...
        for recordContainerCfg in cfgRoot.findall(RECORD_CONTAINER_CFG_ELEMENT_NAME):
            try:
                ...
                try:
                    RecordContainers[containerID].recordTime = time.strptime(recordContainerCfg.attrib['recordTime'], '%H:%M') 
                except:
                    RecordContainers[containerID].recordTime = None
...
def update(PDI):
    ...
    for rc in RecordContainers:
        ...
        #recording by time stamp if configured
        if (RecordContainers[rc].recordTime != None):
            ...
            #save records once every 24h and use lastRecordTimestamp to avoid multiple logging within one minute
            if (RecordContainers[rc].recordTime == currentTime) and ((time.time() - RecordContainers[rc].lastRecordTimestamp) > 60):
        ...
        elif ( (time.time() - RecordContainers[rc].lastRecordTimestamp) >= RecordContainers[rc].interval ):
            RecordContainers[rc].lastRecordTimestamp = time.time()
            RecordContainers[rc].saveDataRecord()

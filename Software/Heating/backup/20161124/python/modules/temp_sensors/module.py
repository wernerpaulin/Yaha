...
def update(PDI):
    ...
    #abort update if simulation is active => temperature values will be directly written in simulation programs (no conversion from IO-channels)
    try:
        if (getattr(PDI, IO_SIMULATION_ACTIVE_PDI_TAG) == 1):
            return
    except:
        pass

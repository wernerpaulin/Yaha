...
class burnerManager:
    ...
    def __init__(self, PDI):
        ...
        self.simOldSwitchState = None
        self.simLastSwitchTime = 0
        self.simStartTemp = 0
        self.startTimeSwitchOn = 0
    ...
    def update(self, PDI):
        ...
        if (self.inputs.switchedOn.value() == 1):
            ...
            self.startTimeSwitchOn = time.time()    #updating will be stopped when switched on
    ...
    def sAUTOMATIC(self):
        ...
        if (self.inputs.actTemp.value() > self.status.setTemperature.value() + self.param.settings.hysteresisHigh) and ((time.time() - self.startTimeSwitchOn) > self.param.settings.minOnTime):
    ...
    def simulationLogic(self):
        ...
        self.param.settings.minOnTime = 10

        #burner is immediately switched on when release
        self.inputs.switchedOn.value(self.outputs.release.value())

        #detect change of switch on/off
        if (self.inputs.switchedOn.value() != self.simOldSwitchState):
            self.simOldSwitchState = self.inputs.switchedOn.value()
            self.simLastSwitchTime = time.time()                            #time stamp of last switch toggle
            self.simStartTemp = self.inputs.actTemp.value()                 #base temperature for simulation curve

        #simulate temperature behavior
        if (self.inputs.switchedOn.value() == 1):
            #PT2 simulation up to 1.5x maximum temperature
            self.inputs.actTemp.value( self.simStartTemp + 
                                       ( (1.5 * self.param.settings.maximumTemperature - self.simStartTemp) * 
                                              math.pow((1-math.exp(-(time.time()-self.simLastSwitchTime) / self.param.simulation.settings.burnerTemperaturePtTau)), 
                                                                    self.param.simulation.settings.temperaturePtOrder)) )
        else:
            #Inverted PT2 with 3x of order to have a slow ramp down
            self.inputs.actTemp.value( self.simStartTemp - 
                                       ( (self.simStartTemp - self.param.simulation.settings.environmentalTemperature) * 
                                              math.pow((1-math.exp(-(time.time()-self.simLastSwitchTime) / self.param.simulation.settings.burnerTemperaturePtTau)), 

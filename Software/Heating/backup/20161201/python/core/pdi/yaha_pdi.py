...
class ProcessDataImage:
    ...
    def addItemsFromFile(self, pdiDefinitionFile):
        ...
        #check if a remanent configuration exists for this process tag and overrite init value of this variable is (still) configured as remanent
    ...
    def read(self, tagsToReadList = None):
        ...
        for processTagName in tagsToReadList:
            try:    #watch out for wrongly given tags
                ...
                print("Yaha_pdi read: process tag <{0}> error: {1}".format(processTagName, e))
        ...
        for processTagName in tagsToWriteList:
            try:    #watch out for wrongly given tags
                ...
                if (self.__internalPDI[processTagName].valueType == VALUE_TYPE_REAL):
                    ...
                    #check whether value has been changed -> used for writing remanent data only when changed 
                    if (self.__internalPDI[processTagName].valueREAL != processTagValue):
                        valueChanged = True
                ...
                if (self.__internalPDI[processTagName].isRemanent == REMANENT_TRUE) and (valueChanged == True):
                    try:
                        ...
                        #with Python 3 the folling lines need to be commented otherwise xml file header will be added twice
                ...
                print("Yaha_pdi write: process tag <{0}> error: {1}".format(processTagName, e))

...
class boilerManager:
    ...
    def __init__(self, PDI):
        ...
        self.legionellaProtectionStartTime = time.time()
        
        self.simTemperatureP1 = core.modules.filter.pt1()
    ...
    def sAUTOMATIC(self):
        ...
        try:
            ...
            startTime = datetime.datetime.strptime(self.param.operatingModes.modes.automatic.morningStartTime.value(), '%H:%M')
            self.morningStartTimeMidnightMinutes = startTime.hour * 60 + startTime.minute

            endTime = datetime.datetime.strptime(self.param.operatingModes.modes.automatic.morningEndTime.value(), '%H:%M')
            self.morningEndTimeMidnightMinutes = endTime.hour * 60 + endTime.minute
        ...
        if (self.hotwaterPreparationRelease == False):
            ...
            try:
                #convert start and end time into minutes since midnight
                startTime = datetime.datetime.strptime(self.param.operatingModes.modes.automatic.eveningStartTime.value(), '%H:%M')
                self.eveningStartTimeMidnightMinutes = startTime.hour * 60 + startTime.minute
    
                endTime = datetime.datetime.strptime(self.param.operatingModes.modes.automatic.eveningEndTime.value(), '%H:%M')
                self.eveningEndTimeMidnightMinutes = endTime.hour * 60 + endTime.minute
        ...
        if (self.inputs.hotwaterActTemp.value() >= self.param.settings.legionellaProtectionTemperature):
            if ((time.time() - self.legionellaProtectionStartTime) > self.param.settings.legionellaProtectionHeatDuration):
    ...
    def simulationLogic(self):
        ...
        self.inputs.hotwaterActTemp.value(self.simTemperatureP1.update(self.simTargetTemp, self.param.simulation.settings.boilerTemperaturePtTau, 1))

...
class roomManager:
    ...
    def sAUTOMATIC(self):
        ...
        if (self.param.operatingModes.modes.automatic.globalMode.value() == GLOBAL_MODE_ON) or (self.param.operatingModes.modes.automatic.globalMode.value() == GLOBAL_MODE_NOT_AT_HOME):
            ...
            try:
                #merge date and time string
                datestring = self.param.operatingModes.modes.automatic.notAtHomeStartDate.value() + " " + self.param.operatingModes.modes.automatic.notAtHomeStartTime.value()
                #parse string and create a datetime object
                startDate = datetime.datetime.strptime(datestring, '%Y-%m-%d %H:%M')
                startDayOfYear = startDate.timetuple().tm_yday
                startTimeMidnightMinutes = startDate.hour * 60 + startDate.minute
                "TODO: Kalender und Uhrzeit einbauen: startDayOfYear, startTimeMidnightMinutes"
            ...
            #ROOM_MODE_AUTOMATIC
                ...
                if (self.currentWeekdayIndex == SATURDAY_INDEX) or (self.currentWeekdayIndex == SUNDAY_INDEX):
                    try:
                        ...
                        startTime = datetime.datetime.strptime(self.param.operatingModes.modes.automatic.weekendStartTime.value(), '%H:%M')
                        self.startTimeMidnightMinutes = startTime.hour * 60 + startTime.minute
                        
                        endTime = datetime.datetime.strptime(self.param.operatingModes.modes.automatic.weekendEndTime.value(), '%H:%M')
                        self.endTimeMidnightMinutes = endTime.hour * 60 + endTime.minute
                ...
                #WEEKDAYS
                    ...
                    try:
                        #convert start and end time into minutes since midnight
                        startTime = datetime.datetime.strptime(self.param.operatingModes.modes.automatic.weekdayStartTime.value(), '%H:%M')
                        self.startTimeMidnightMinutes = startTime.hour * 60 + startTime.minute
                        
                        endTime = datetime.datetime.strptime(self.param.operatingModes.modes.automatic.weekdayEndTime.value(), '%H:%M')
                        self.endTimeMidnightMinutes = endTime.hour * 60 + endTime.minute

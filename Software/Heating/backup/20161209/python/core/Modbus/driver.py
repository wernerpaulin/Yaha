...
ALARM_MODBUS_NO_CONNECTION_PDI_TAG = "alarmModbusNoConnection"
ALARM_IO_SIMULATION_PDI_TAG = "alarmIOsimulationActive"
ALARM_ACTIVE = 1
ALARM_INACTIVE = 0

MODBUS_IO_SIMULATION_ACTIVE_PDI_TAG = "ioSimulation"
...
class main():
    ...
    def start(self):
        ...
        try:
            if (getattr(self.PDI, MODBUS_IO_SIMULATION_ACTIVE_PDI_TAG) == 1):
                print("WARNING: Modbus IO simulation is active")
                setattr(self.PDI, ALARM_IO_SIMULATION_PDI_TAG, ALARM_ACTIVE) #set alarm
                return
        except:
            pass
...
class modbusClient():
    ...
    def __init__(self,ipAdr, portNr, PDI):
        ...
        self.modbusHandlerThreadID = 0
        self.modbusWaitToConnectThreadID = 0
    ...
    def connect(self):
        #set connection failed alarm per default. Will be reset once connection is established
        try:
            setattr(self.PDI, ALARM_MODBUS_NO_CONNECTION_PDI_TAG, ALARM_ACTIVE)
        except:
            pass
                
        #before starting another communication thread, check whether the old one has been terminated (= thread function finished)
        try:
            if (self.modbusWaitToConnectThreadID.isAlive() == True):
                return
        except:
            #thread handle invalid = thread not yet started => also ok. Continue with starting a new thread
            pass

        #as connect might take some time, call it in a thread to avoid blocking the cyclic system        
        try:
            self.modbusWaitToConnectThreadID = threading.Thread(target=self.waitingToConnectThread)
            self.modbusWaitToConnectThreadID.start()
            return
        
        except Exception as e:
            print("Modbus (IP: {0}): starting Modbus connect thread failed: {1}".format(self.modbusIPadr, e))
            return  
        
        
    def waitingToConnectThread(self):            
    ...
    def readInputs(self):
        #do not read inputs if connection is not established
        try:
            if (self.modbusWaitToConnectThreadID.isAlive() == True):
                return
        except:
        ...
        try:
            ...
            if (self.modbusHandlerThreadID.isAlive() == False):
                #print("Modbus communication time: {0}".format(time.time() - self.modbusStartTime))
                #clear any pending alarm
                try:
                    setattr(self.PDI, ALARM_MODBUS_NO_CONNECTION_PDI_TAG, ALARM_INACTIVE)
                except:
                    pass
                ...
                #indicate connection problem with alarm
                try:
                    setattr(self.PDI, ALARM_MODBUS_NO_CONNECTION_PDI_VAR, ALARM_ACTIVE)
                except:
                    pass
    ...
    def writeOutputs(self):
        #do not write outputs if connection is not established
        try:
            if (self.modbusWaitToConnectThreadID.isAlive() == True):
                return
        except:
        ...
        #before starting another communication thread, check whether the old one has been terminated (= thread function finished)
        try:
            if (self.modbusHandlerThreadID.isAlive() == True):
                return
        except:
            #thread handle invalid = thread not yet started => also ok. Continue with starting a new thread
            pass
        ...
        try:
            ...
            self.modbusHandlerThreadID = threading.Thread(target=self.modbusHandlerThread)
            self.modbusStartTime = time.time()
            self.modbusHandlerThreadID.start()
            return

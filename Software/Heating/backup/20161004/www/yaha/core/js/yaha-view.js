...
var ModuleViewOrder = []				
var ModuleIdOrderList = {}				/* nameID: order number */  		
	...
	getAvailableLanguages(function(availableLanguages) /* call back function */
			{
				...
				//get available modules on target
				getModuleList(ActiveLanguage, function(moduleNameList)
						{				
							/* store module names in global list */
							ModuleNameList = moduleNameList;

							//GUI load time optimization: see whether predefined module exists and load it straight away. Once loaded all other modules are loaded in the background
							if (YAHA_INIT_MODULE in ModuleNameList)
							{
								getModuleContent(YAHA_INIT_MODULE, ActiveLanguage, function(moduleNameID, orderNr, htmlStream)
										{				
											ModuleContentList[moduleNameID] = htmlStream	//save html content of module in list for later display
											
											showModuleContent(YAHA_INIT_MODULE)
										
											/* reload all modules */
											loadAllModules(ActiveLanguage, function() {})										
										})
							}
							//init module not defined => load all modules and show a default module
							else
							{
								/* reload all modules */
								loadAllModules(ActiveLanguage, function()
										{
											showModuleContent(ModuleViewOrder[0]) //show first module
										}) /* loadAllModules */										
							}
						})
			}) /* getAvailableLanguages */
	
	//add event handler to open main navigation panel in case the user clicks on the header
...
function loadAllModules(activeLanguage, callbackFunction)
{
	...
	{
		getModuleContent(moduleNameID, ActiveLanguage, function(moduleNameID, orderNr, htmlStream)
				{
					ModuleContentList[moduleNameID] = htmlStream	//save html content of module in list for later display
					ModuleIdOrderList[moduleNameID] = orderNr
					...
					{
						//build module view order depending on order number of module
						ModuleViewOrder = []
						ModuleViewOrder = getSortedKeys(ModuleIdOrderList)
...
function getModuleContent(nameID, language, callbackFunction)
{
	...
	jQuery.post(CGI_SCRIPT_NAME_YAHA_VIEW, cgiData,
			function(xmlModuleDataFromServer,status)
		    {
				...
				nameID = $id.text()
				//get order number of uploaded module for further referencing in ModuleViewOrder
				$order = $xmlroot.find( "info>order" );				
				orderNr = parseFloat($order.text())
				...
				callbackFunction(nameID, orderNr, htmlDataFromServer)
		    });
}
...
function showModuleContent(moduleNameId)
{
	...
	if (moduleNameId == "") 
	{
		console.log('showModuleContent: moduleNameId not defined')
		return
	}
	...
	//load content of submodule into page content including javascript functions e.g. moduleInit()
	$(".yaha-ui-content").html(ModuleContentList[moduleNameId]).promise().done(function()	//.promise().done() is a kind of callback when the entire content has been loaded
			{
				...
				{
					console.log('showModuleContent: no moduleInit defined for this module or error in function: ' + err)
				}
...
function yvSwitchLanguage(languageID)
{
	...
	//get available modules on target with new language
	getModuleList(ActiveLanguage, function(moduleNameList)
			{				
				/* store module names in global list */
				ModuleNameList = moduleNameList;	

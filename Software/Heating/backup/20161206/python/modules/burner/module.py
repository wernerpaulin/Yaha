...
class burnerManager:
    ...
    def simulationLogic(self):
        ...
        if (self.inputs.switchedOn.value() == 1):
            self.simTargetTemp = self.status.setTemperature.value()
        else:
        ...
        self.inputs.actTemp.value( ((self.simSamplingTime * self.simTargetTemp) + (self.param.simulation.settings.burnerTemperaturePtTau * self.inputs.actTemp.value())) / (self.simSamplingTime + self.param.simulation.settings.burnerTemperaturePtTau) )

...
class boilerManager:
    ...
    def __init__(self, PDI):
        ...
        self.morningStartTimeMidnightMinutes = None
        self.morningEndTimeMidnightMinutes = None
        self.eveningStartTimeMidnightMinutes = None
        self.eveningEndTimeMidnightMinutes = None
        self.currentTimeMidnightMinutes = None
        self.hotwaterPreparationRelease = False
        
        self.simTargetTemp = 0
        self.simLastCallTime = time.time()
        self.simSamplingTime = 0
    ...
    def sAUTOMATIC(self):
        ...
        try:
            ...
            self.morningStartTimeMidnightMinutes = int(self.param.operatingModes.modes.automatic.morningStartTime.value().split(":")[HOUR_INDEX]) * 60 + int(self.param.operatingModes.modes.automatic.morningStartTime.value().split(":")[MINUTE_INDEX])
            self.morningEndTimeMidnightMinutes   = int(self.param.operatingModes.modes.automatic.morningEndTime.value().split(":")[HOUR_INDEX]) * 60 + int(self.param.operatingModes.modes.automatic.morningEndTime.value().split(":")[MINUTE_INDEX])
            
            #check whether current time is within the morning phase
            if (self.currentTimeMidnightMinutes >= self.morningStartTimeMidnightMinutes) and (self.currentTimeMidnightMinutes <= self.morningEndTimeMidnightMinutes):
        ...
        if (self.hotwaterPreparationRelease == False):
            #print(self.hotwaterPreparationRelease)
            try:
                #convert start and end time into minutes since midnight
                self.eveningStartTimeMidnightMinutes = int(self.param.operatingModes.modes.automatic.eveningStartTime.value().split(":")[HOUR_INDEX]) * 60 + int(self.param.operatingModes.modes.automatic.eveningStartTime.value().split(":")[MINUTE_INDEX])
                self.eveningEndTimeMidnightMinutes   = int(self.param.operatingModes.modes.automatic.eveningEndTime.value().split(":")[HOUR_INDEX]) * 60 + int(self.param.operatingModes.modes.automatic.eveningEndTime.value().split(":")[MINUTE_INDEX])
                
                #check whether current time is within the morning phase
                if (self.currentTimeMidnightMinutes >= self.eveningStartTimeMidnightMinutes) and (self.currentTimeMidnightMinutes <= self.eveningEndTimeMidnightMinutes):
                    self.hotwaterPreparationRelease = True
                else:
                    self.hotwaterPreparationRelease = False
        ...
        if (self.hotwaterPreparationRelease == True):
            ...
            #On sunday higher set temperature to kill legionella  
            if (datetime.datetime.today().weekday() == SUNDAY_INDEX):
                setTemperature = self.param.settings.legionellaProtectionTemperature
            else:
                setTemperature = self.param.operatingModes.modes.automatic.setTemperature.value()

            #two point controller
            if (self.inputs.hotwaterActTemp.value() < setTemperature - self.param.settings.hysteresisLow):
                self.outputs.burnerSetTemp.value(setTemperature)
                self.outputs.pumpRelease.value(1)

            if (self.inputs.hotwaterActTemp.value() > setTemperature + self.param.settings.hysteresisHigh):
    ...
    def simulationLogic(self):
        ...
        self.simSamplingTime = max(time.time() - self.simLastCallTime, 0)
        self.simLastCallTime = time.time()
        
        #temp = (dt * targetTemp + Tau * temp) ( (dt + Tau)    FIR filter
        self.inputs.hotwaterActTemp.value( ((self.simSamplingTime * self.simTargetTemp) + (self.param.simulation.settings.boilerTemperaturePtTau * self.inputs.hotwaterActTemp.value())) / (self.simSamplingTime + self.param.simulation.settings.boilerTemperaturePtTau) )
